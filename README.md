...
## 普通版
- vue博客 ui：[cap-blog-ui](https://gitee.com/bweird/cap-blog-ui)
...
- 如果不喜欢工作流，可以直接删除 cap-activiti模块  
并在cap-web/pom.xml、父 pom.xml 删除依赖以及模块，删除application.java 中对activiti的扫描即可完全删除工作流模块。
...
- 19/4/30 **升级admin监控**
**博客采用vue前后分离基于jwt鉴权，博客单独分离出一个项目地址:[cap-blog-ui](https://gitee.com/bweird/cap-blog-ui)**
## 头像说明
```
...
mvn clean package
mvn package
java -jar cap-web.jar
```
- db使用mysql，项目数据库在 根目录db文件夹下，
导入数据库后 设定数据库用户名密码 在文件lenosp\cap-web\src\main\resources\application.yml中
